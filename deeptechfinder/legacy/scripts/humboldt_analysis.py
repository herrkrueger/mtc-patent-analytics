#!/usr/bin/env python3
"""
Humboldt University of Berlin Patent Analysis with EPO OPS
Analyzes all Humboldt University patents from DeepTechFinder, extracts German priorities and industry collaborators
Based on TU Dresden analysis methodology
"""
...
print("🎯 HUMBOLDT UNIVERSITY OF BERLIN PATENT ANALYSIS")
print("=" * 60)
...
def normalize_applicant_name(name):
    ...
    # Humboldt University normalization patterns
    replacements = {
        ...
        'HUMBOLDT-UNIVERSITY OF BERLIN': 'HUMBOLDT UNIVERSITY BERLIN',
        'BERLIN HUMBOLDT UNIVERSITY': 'HUMBOLDT UNIVERSITY BERLIN',
    }
...
def main():
    ...
    # Load Humboldt University patents
    print("\n📂 Loading Humboldt University patents...")
    try:
        patents_df = pd.read_csv('./output/patent_technology_list.csv')
        humboldt_patents = patents_df[patents_df['University'] == 'Humboldt University of Berlin']
        granted_patents = humboldt_patents[humboldt_patents['Patent_status'] == 'EP granted']
        
        print(f"✅ Found {len(humboldt_patents)} total Humboldt University patents")
        ...
        # Show technical field distribution
        tech_fields = granted_patents['Technical_field'].value_counts()
        print(f"🔬 Top technical fields:")
        for field, count in tech_fields.head(3).items():
    ...
    # Dataset size notification
    print(f"\n⚡ ANALYSIS SCOPE:")
    print(f"   Processing {len(granted_patents)} granted Humboldt University patents")
    ...
    for idx, (_, row) in enumerate(granted_patents.iterrows(), 1):
        ...
        if biblio_data:
            ...
            university_terms = ['university', 'universität', 'humboldt', 'berlin']
    ...
    # Generate summary
    print(f"\n📊 HUMBOLDT UNIVERSITY ANALYSIS COMPLETE")
    print(f"=" * 60)
    ...
    if results:
        ...
        results_file = "./output/humboldt_complete_analysis.csv"
        ...
        applicants_df = pd.DataFrame({
            ...
            'type': ['University' if any(term in app.lower() for term in ['university', 'universität', 'humboldt', 'berlin']) 
                    else 'Industry/Other' for app in sorted(all_applicants)]
        })
        applicants_file = "./output/humboldt_applicants.csv"
        ...
        inventors_file = "./output/humboldt_inventors.csv"
        ...
        if german_priorities:
            ...
            priorities_file = "./output/humboldt_german_priorities.csv"
