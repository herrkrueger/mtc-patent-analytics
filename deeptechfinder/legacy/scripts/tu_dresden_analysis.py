#!/usr/bin/env python3
"""
TU Dresden Patent Analysis with EPO OPS
Analyzes all TU Dresden patents from DeepTechFinder, extracts German priorities and industry collaborators
Fixed inventor normalization to handle comma variations
"""
...
print("🎯 TU DRESDEN PATENT ANALYSIS")
print("=" * 50)
...
def normalize_applicant_name(name):
    ...
    # TU Dresden normalization patterns
    replacements = {
...
def extract_patent_data(biblio_data):
    ...
    try:
        ...
        # Extract inventors (prefer original format, improved normalization)
...
def main():
    ...
    # Load TU Dresden patents (only Technische Universität Dresden, not Fachhochschule)
    print("\n📂 Loading TU Dresden patents...")
    try:
        patents_df = pd.read_csv('./output/patent_technology_list.csv')
        tu_dresden_patents = patents_df[patents_df['University'] == 'Technische Universität Dresden']
        granted_patents = tu_dresden_patents[tu_dresden_patents['Patent_status'] == 'EP granted']
        
        print(f"✅ Found {len(tu_dresden_patents)} total TU Dresden patents")
        ...
        filing_dist = granted_patents['Filing_Year'].value_counts().sort_index()
        print(f"📊 Patents by decade:")
        for decade in [1980, 1990, 2000, 2010, 2020]:
            decade_count = len(granted_patents[(granted_patents['Filing_Year'] >= decade) & (granted_patents['Filing_Year'] < decade + 10)])
            if decade_count > 0:
                print(f"   {decade}s: {decade_count} patents")
        
    except Exception as e:
        print(f"❌ Error loading data: {e}")
        return
    
    # Given the large size (265 patents), ask for confirmation
    print(f"\n⚠️  LARGE DATASET NOTICE:")
    print(f"   This analysis will process {len(granted_patents)} granted patents")
    print(f"   Estimated time: ~{len(granted_patents) * 3 / 60:.1f} minutes (with 3s delays)")
    print(f"   Rate limiting: 2 seconds between requests for EPO OPS compliance")
    
    # For demonstration, process first 50 patents to show methodology
    # Comment out the line below to process all patents
    granted_patents = granted_patents.head(50)
    print(f"\n🔬 DEMONSTRATION MODE: Analyzing first {len(granted_patents)} patents")
    print(f"💡 Remove .head(50) in script to process all {len(patents_df[patents_df['University'] == 'Technische Universität Dresden'][patents_df['Patent_status'] == 'EP granted'])} patents")
    ...
    for idx, (_, row) in enumerate(granted_patents.iterrows(), 1):
        ...
        if biblio_data:
            ...
            # Normalize and collect inventors (fixed normalization)
            ...
            university_terms = ['university', 'universität', 'technische', 'tu ', 'dresden']
    ...
    print(f"🔬 Total unique inventors: {len(all_inventors)} (fixed normalization)")
    ...
    print(f"\n🔬 SAMPLE INVENTORS (first 10, properly normalized):")
    for inventor in sorted(all_inventors)[:10]:
        print(f"  • {inventor}")
    if len(all_inventors) > 10:
        print(f"  • ... and {len(all_inventors) - 10} more inventors")
    ...
    if german_priorities:
        print(f"\n🇩🇪 SAMPLE GERMAN PRIORITY PATENTS (first 5):")
        for gp in german_priorities[:5]:
            print(f"  {gp['german_priority']} → {gp['ep_patent']} | {', '.join(gp['applicants'][:2])}{'...' if len(gp['applicants']) > 2 else ''}")
        if len(german_priorities) > 5:
            print(f"  ... and {len(german_priorities) - 5} more priority relationships")
    
    # Export results
    if results:
        # Export detailed results
        results_df = pd.DataFrame(results)
        results_file = "./output/tu_dresden_complete_analysis.csv"
        ...
        applicants_df = pd.DataFrame({
            ...
            'type': ['University' if any(term in app.lower() for term in ['university', 'universität', 'technische', 'tu ', 'dresden']) 
                    else 'Industry/Other' for app in sorted(all_applicants)]
        })
        applicants_file = "./output/tu_dresden_applicants.csv"
        ...
        inventors_file = "./output/tu_dresden_inventors.csv"
        inventors_df.to_csv(inventors_file, index=False)
        print(f"🔬 Inventor list: {inventors_file} (fixed normalization)")
        
        # Export German priorities
        if german_priorities:
            priorities_df = pd.DataFrame(german_priorities)
            priorities_file = "./output/tu_dresden_german_priorities.csv"
            priorities_df.to_csv(priorities_file, index=False)
            print(f"🇩🇪 German priorities: {priorities_file}")
    
    print(f"\n✅ TU DRESDEN ANALYSIS COMPLETE!")
    print(f"🕐 Finished: {datetime.now().strftime('%H:%M:%S')}")
    print(f"\n💡 TO PROCESS ALL {len(patents_df[patents_df['University'] == 'Technische Universität Dresden'][patents_df['Patent_status'] == 'EP granted'])} PATENTS:")
    print(f"   Remove the .head(50) line in the script")
